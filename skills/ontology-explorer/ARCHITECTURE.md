...
Shipment              CreditCard           Encounter            Review
Contract              Loan                 LabResult            Coupon
Inventory                                  InsuranceClaim
```
...
Supplier ──1:N──→ Contract                    Account ──1:N──→ Transaction (in)
Warehouse ──1:N──→ Shipment                   FiCustomer ──1:N──→ CreditCard
Warehouse ──1:N──→ Inventory                  FiCustomer ──1:N──→ Loan
PurchaseOrder ──1:N──→ Shipment               Account ──1:N──→ Loan
ScProduct ──1:N──→ Inventory

healthcare                                    ecommerce
──────────                                    ─────────
Patient ──1:N──→ Diagnosis                    EcomUser ──1:N──→ EcomOrder
Patient ──1:N──→ Encounter                    Merchant ──1:N──→ EcomProduct
Patient ──1:N──→ LabResult                    EcomProduct ──1:N──→ EcomOrder
Patient ──1:N──→ InsuranceClaim               EcomProduct ──1:N──→ Review
Provider ──1:N──→ Diagnosis                   EcomUser ──1:N──→ Review
Provider ──1:N──→ Encounter                   EcomOrder ──1:N──→ Coupon
Encounter ──1:N──→ LabResult
Encounter ──1:N──→ InsuranceClaim
```
...
│   ├── objectTypes: [...]           #   26 个对象类型定义
│   ├── linkTypes: [...]             #   26 个关系类型定义
│   ├── interfaceTypes: [...]        #    2 个接口类型定义
│   └── actionTypes: [...]           #   10 个操作类型定义
└── instances/
    ├── supply_chain.json            #   7 种类型共 296 条实例
    ├── finance.json                 #   6 种类型共 257 条实例
    ├── healthcare.json              #   7 种类型共 618 条实例
    └── ecommerce.json               #   6 种类型共 302 条实例
                                     #   总计 1473 条实例
```
